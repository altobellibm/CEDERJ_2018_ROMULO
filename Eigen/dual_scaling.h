...
namespace ds {
    ...
    public:
        ...
        rank_order_data(const Eigen::EigenBase<Derived> &data) :
            ...
            std::ofstream file;
            file.open("EOriginal.txt");
                file << E_;
            file.close();


            E_ *= -2;
            file.open("Emenos2.txt");
                file << E_;
            file.close();

            E_ += matrix_type::Constant(E_.rows() ,E_.cols(), E_.cols() + 1);

            file.open("Emaisconstant.txt");
                file << E_;
            file.close();
        }
